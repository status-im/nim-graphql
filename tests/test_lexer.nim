...
template scanEscError(input, expect: string) =
  var lex = initLex("\"" & input & "\"")
  lex.next()
  check lex.tok == tokError
  check lex.err.message == expect

template scanEsc(input, expect: string) =
  var lex = initLex("\"" & input & "\"")
  lex.next()
  check lex.tok in {tokString, tokBlockString}
  check lex.token == expect

suite "full range Unicode(UTF-8) support":
  test "too much digits":
    scanEscError("\\u12345", "Invalid unicode sequence \'12345\'")
    scanEscError("\\u{1234567}", "Invalid unicode sequence \'1234567\'")

  test "good digits":
    scanEsc("\\u000A", "\u{000A}")
    scanEsc("\\uD7FF", "\u{D7FF}")
    scanEsc("\\u{D7FF}", "\u{D7FF}")
  ...
  test "invalid sequence":
    ...
    scanEscError("\\u{D800}", "Invalid unicode sequence 'D800'")
    scanEscError("\\u{DFFF}", "Invalid unicode sequence 'DFFF'")
    scanEscError("\\u{110000}", "Invalid unicode sequence '110000'")
